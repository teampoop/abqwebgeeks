  ...
  b.Mason.settings = {
    ...
    _filterFindBricks: function (a) {
      ...
      for (var j = 0, k = a.length; j < k; j++) {
        ...
        if (e === 1) {
          this._placeBrick(d, this.colYs);
        }
        else {
          f = this.cols + 1 - e, g = [];
          for (i = 0; i < f; i++) {
            h = this.colYs.slice(i, i + e), g[i] = Math.max.apply(Math, h);
          }
      ...
      for (j = 0, k = this.styleQueue.length; j < k; j++) {
        o = this.styleQueue[j], o.$el[m](o.style, n);
      }
    ...
    if (typeof a == "string") {
      ...
      this.each(function () {
        ...
        if (!d) {
          return b.error("cannot call methods on masonry prior to initialization; attempted to call method '" + a + "'");
        }
        if (!b.isFunction(d[a]) || a.charAt(0) === "_") {
          return b.error("no such method '" + a + "' for masonry instance");
        }
        d[a].apply(d, c)
      })
    }
